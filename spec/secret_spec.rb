...
describe SecretStore::Secret do
  ...
  describe 'class methods' do
    describe '#new' do
      it 'creates valid, decryptable object' do
        secret = described_class.new(example_label, example_iv, example_pbkdf2_salt, example_crypted_text,
                                     example_auth_tag)
    ...
    describe '#create_from_plaintext' do
      it 'creates a valid, decryptable object' do
        secret = described_class.create_from_plaintext(example_label, example_plaintext, example_checksum)
        expect(secret.decrypt_text(example_checksum)).to eql example_plaintext
      end
  ...
  describe 'instance methods' do
    subject do
      described_class.new(example_label, example_iv, example_pbkdf2_salt, example_crypted_text, example_auth_tag)
    end
    ...
    describe '#decrypt_text' do
      ...
      it 'raises error when crypted text has been tampered with' do
        ...
        tampered_secret = described_class.new(example_label, example_iv, example_pbkdf2_salt,
        ...
        copy_cipher[-1] = (copy_cipher[-1].ord ^ 4).chr # xor a bit on last chaacter
        fixed_secret = described_class.new(example_label, example_iv, example_pbkdf2_salt,
                                           Base64.urlsafe_encode64(copy_cipher), example_auth_tag)
        expect(fixed_secret.decrypt_text(example_checksum)).to eql example_plaintext
      end

      it 'raises error when label is changed' do
        tampered_secret = described_class.new('new_label', example_iv, example_pbkdf2_salt,
    ...
    describe '#replace_text' do
      ...
      it 'changes iv, pbkdf2_salt, crypted text and auth_tag even if key and message are the same' do
        ...
        expect(subject.iv).not_to eql example_iv
        expect(subject.crypted_text).not_to eql example_crypted_text
        expect(subject.pbkdf2_salt).not_to eql example_pbkdf2_salt
        expect(subject.auth_tag).not_to eql example_auth_tag
      end
    end
