...
module SecretStore
  ...
  class Secret
    include CoreMethods
    extend CoreMethods
    include RecordValidation
    extend RecordValidation
    ...
    def initialize(label, iv_b64, pbkdf2_salt_b64, crypted_text_b64, auth_tag_b64)
      validate_secret(label, iv_b64, pbkdf2_salt_b64, crypted_text_b64, auth_tag_b64)
      @label = label
      @iv = iv_b64
    ...
    def replace_text(new_text, checksum)
      replacement = self.class.create_from_plaintext(label, new_text, checksum)
      @iv = replacement.iv
      @pbkdf2_salt = replacement.pbkdf2_salt
      @crypted_text = replacement.crypted_text
      @auth_tag = replacement.auth_tag
      self
    end
    ...
    # @param [Hash] attributes as generated by .to_h
    # @return [SecretStore::Secret] new object
    def self.from_h(attributes)
      validate_fields(attributes, %i[label iv pbkdf2_salt crypted_text auth_tag])

      new(attributes[:label], attributes[:iv], attributes[:pbkdf2_salt], attributes[:crypted_text],
          attributes[:auth_tag])
    end
  end
end
