...
module SecretStore
  ...
  class Password
    ...
    def initialize(bcrypt_salt, pbkdf2_salt, test_encryption)
      validate_password(bcrypt_salt, pbkdf2_salt, test_encryption)
    ...
    def self.from_h(attributes)
      validate_fields(attributes, %i[bcrypt_salt pbkdf2_salt test_encryption])
    ...
    def verify_test_encryption(passcode, test_text)
      key = key_from_checksum(passcode, decode_bytes(pbkdf2_salt))
      iv_b64, crypted_text_b64, auth_tag_b64 = validate_test_encryption(test_text)
      begin
